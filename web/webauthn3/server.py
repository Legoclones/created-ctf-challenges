...
def handle_webauthn_reg(reg: dict) -> tuple[int, dict]:
    try:
        ...
        # ensure the credential ID matches the one in the registration data
        if verified.credential_id.hex() != base64url_to_bytes(reg['id']).hex():
            return (-1, {})
...
def login():
    if request.method == 'GET':
        ...
        # check for flag
        if credentials[users['admin']]['sign'] < 10:
            # reset
            credentials[users['admin']]['sign'] = 10

            msg = f"Flag: {FLAG}"
        else:
            msg = "Login successful! But no flag"
